...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        ...
        print("Run again as 'python3 format_sheets_songs.py path/to/exported.csv path/to/output.csv'")
        exit()
    ...
    with open(source_csv_path, 'r', newline='') as source_csv:
        ...
        # Handle header
        next(csvreader)
        new_data.append(["Song", "Artist", "Album", "Genre"])
        ...
        for r, row in enumerate(csvreader):
            ...
            new_data.append([song, artist, album, genres])
